...
def create_spend_tracking_sheet(cleaned_data):
    try:
        ...
        spend_summary = spend_by_keyword.groupby(['Portfolio', 'Campaign Name', 'Targeting']).agg(
            ...
            avg_cpc=('weekly_cpc', 'mean'),
            avg_acos=('weekly_acos', 'mean')
        ).reset_index()
        ...
        # Calculate the last week's spend, CPC, and ACOS
        last_week_spend = last_week_data.groupby(['Portfolio', 'Campaign Name', 'Targeting']).agg(
            ...
            last_week_acos=('ACOS', 'mean')
        ).reset_index()

        # Merge the average spend and last week's spend into a single DataFrame
        spend_tracking = spend_summary.merge(
            last_week_spend.rename(columns={'last_week_spend': 'Last Week Spend', 'last_week_cpc': 'Last Week CPC', 'last_week_acos': 'Last Week ACOS'}),
        ...
        spend_tracking = spend_tracking[['Portfolio', 'Campaign Name', 'Targeting', '4 Week Avg Spend', 'Last Week Spend', 'Status', 
                                         'avg_cpc', 'last_week_cpc', 'avg_acos', 'last_week_acos']]
        spend_tracking.rename(columns={'avg_cpc': '4 Week Avg CPC', 'last_week_cpc': 'Last Week CPC', 'avg_acos': '4 Week Avg ACOS', 'last_week_acos': 'Last Week ACOS'}, inplace=True)
        ...
        for sheetname in workbook.sheetnames:
            ...
            if sheetname == 'Spend Tracking':
                for col_num, column in enumerate(worksheet.iter_cols(min_row=2, min_col=1), 1):
                    ...
                    if header_value in ['4 Week Avg Spend', 'Last Week Spend', '4 Week Avg CPC', 'Last Week CPC', '4 Week Avg ACOS', 'Last Week ACOS']:
                        for cell in column:
