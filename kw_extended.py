...
# New function to create the 'Spend Tracking' sheet
def create_spend_tracking_sheet(cleaned_data):
    try:
        ...
        recent_data = cleaned_data[(cleaned_data['Date'] > start_date) & (cleaned_data['Date'] <= end_date)]
        last_week_data = cleaned_data[(cleaned_data['Date'] > last_week_start) & (cleaned_data['Date'] <= end_date)]

        spend_by_keyword = recent_data.groupby('Targeting')['Ad Spend'].sum()
        average_spend_4_weeks = spend_by_keyword / 4
        last_week_spend = last_week_data.groupby('Targeting')['Ad Spend'].sum()

        spend_tracking = pd.DataFrame({
            'Keyword': spend_by_keyword.index,
            '4 Week Avg Spend': average_spend_4_weeks,
            'Last Week Spend': last_week_spend
        })

        spend_tracking.fillna(0, inplace=True)
        spend_tracking['Change'] = (spend_tracking['Last Week Spend'] - spend_tracking['4 Week Avg Spend']) / spend_tracking['4 Week Avg Spend']
        spend_tracking['Change'] = spend_tracking['Change'].round(2)
        ...
        spend_tracking = spend_tracking[['Keyword', '4 Week Avg Spend', 'Last Week Spend', 'Status']]
